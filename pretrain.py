...
from config import GPU_DEVICE,ALPHA,SPM_MODEL
...
#             src, tgt = src.split()[:max_seq_len], tgt.split()[:max_seq_len]
            src = spm_model.encode(src, out_type=str, enable_sampling=True, alpha=ALPHA, nbest_size=-1)[:max_seq_len]
            tgt = spm_model.encode(tgt, out_type=str, enable_sampling=True, alpha=ALPHA, nbest_size=-1)[:max_seq_len]
            src_tokens.append(src)
            tgt_tokens.append(tgt)
        print(f'Num invalid line :{cnt}')
        self.src = src_tokens
        self.tgt = tgt_tokens
        self.adt = adt_sents
