        ...
        for batch in train_data:
            ...
            if args.world_size == 1 or (dist.get_rank() == 0):
                if global_step % args.print_every == -1 % args.print_every:
                    ...
                    writer.add_scalar("Loss/train", tr_stat['loss'] / tr_stat['tokens'],  global_step)
                    writer.add_scalar("Acc/train", tr_stat['acc'] / tr_stat['tokens'], global_step)
                    tr_stat = Statistics()
                if global_step % args.eval_every == -1 % args.eval_every:
                    model.eval()
                    max_time_step = 256 if global_step > 2*args.warmup_steps else 30
                    bleus = []
                    print('-'*100)
                    ...
                    writer.add_scalar("Bleu_dev/dev", bleu, global_step)
...
#                         testbleu = sum(testbleus) / len(testbleus)
                        testbleu = bleu
                        writer.add_scalar("Bleu_test/test", testbleu, global_step)
