...
class Beam(object):
    ...
    def update(self, new_states, next_steps):
        ...
        _prev_hyp_idx = torch.tensor([ x[0] for x in candidates]).cuda(torch.device(f'cuda:{GPU_DEVICE}'))
...
def search_by_batch(model, beams, mem_dict):
    ...
    def ready_to_submit(hypotheses):
        ...
        '''
        inp = model.prepare_incremental_input([hyp.seq[-1:] for hyp in hypotheses]).cuda(torch.device(f'cuda:{GPU_DEVICE}'))
#         inp = model.prepare_incremental_input([hyp.seq[-1:] for hyp in hypotheses])
    ...
    while True:
        ...
        # collect mem_dict
        cur_mem_dict = dict()
        indices = torch.tensor(indices).cuda(torch.device(f'cuda:{GPU_DEVICE}'))
