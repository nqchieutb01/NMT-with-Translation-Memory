...
from config import ALPHA,SPM_MODEL,FINE_TUNE,TOPK
if FINE_TUNE:
...
class Vocab(object):
    ...
    def size(self):
        if FINE_TUNE:
            return self.spm_model.get_piece_size()
        else:
            return self.spm_model.get_piece_size()
    ...
    def idx2token(self, x):
        ...
        return self.spm_model.id_to_piece(x)

    def token2idx(self, x):
        if isinstance(x, list):
            return [self.token2idx(i) for i in x]
        return self.spm_model.piece_to_id(x)
...
def batchify(data, vocabs, max_seq_len):
    ...
    if data[0]['mem_sents']:
        ...
        for x in data:
#             print(len(x['mem_sents']))
#             print(len(x['mem_scores']))
#             if len(x['mem_sents'])==6:
#                 print(x['mem_sents'])
#                 print(x['mem_scores'])
...
class DataLoader(object):
    def __init__(self, vocabs, filename, batch_size, for_train, max_seq_len=512, rank=0, num_replica=1):
        self.vocabs = vocabs
        ...
        for line in open(filename).readlines()[rank::num_replica]:
            ...
            src = spm_model.encode(src, out_type=str, enable_sampling=True, alpha=self.alpha, nbest_size=-1)
            tgt = spm_model.encode(tgt, out_type=str, enable_sampling=True, alpha=self.alpha, nbest_size=-1)
...
#             mem_sents.append([ref.split() for ref in mem[:-1:2]])
            mem_sents.append([spm_model.encode(ref, out_type=str, enable_sampling=True, alpha=ALPHA, nbest_size=-1) for ref in mem[:-1:2][:TOPK]])
#             mem_scores.append([float(score) for score in mem[1::2][:TOPK]])
            mem_scores.append([1.0 for score in mem[1::2][:TOPK]])
        self.src = src_tokens
        self.tgt = tgt_tokens
