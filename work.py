...
from config import SPM_MODEL,GPU_DEVICE
...
def validate(device, model, test_data, beam_size=5, alpha=0.6, max_time_step=100, dump_path=None):
    ...
    for batch in tqdm(test_data):
        ...
        if sys_retr:
            ...
            mem_scores.extend(batch.get('all_mem_scores', None).tolist())
...
#     ref_stream = [ re.sub(r'(@@ )|(@@ ?$)', '', ' '.join(o)) for o in ref_stream]
    sys_stream = [re.sub(r'(@@ )|(@@ ?$)', '', spm_model.decode(o)) for o in sys_stream]
    ref_stream = [re.sub(r'(@@ )|(@@ ?$)', '', spm_model.decode(o)) for o in ref_stream]
    ...
    if topk_sys_retr_stream:
        ...
        for i in range(topk):
            sys_retr_stream = [ re.sub(r'(@@ )|(@@ ?$)', '', spm_model.decode(o[i])) for o in topk_sys_retr_stream]
#             sys_retr_stream = [ re.sub(r'(@@ )|(@@ ?$)', '', ' '.join(o[i])) for o in topk_sys_retr_stream]
            lratio = []
            for aa, bb in zip(sys_retr_stream, ref_stream):
                laa = len(aa.split())
                lbb = len(bb.split())
                lratio.append(max(laa/(lbb+1e-12), lbb/(laa+1e-12)))
...
#                         out_line = re.sub(r'(@@ )|(@@ ?$)', '', ' '.join(out_tokens))
                        out_line = re.sub(r'(@@ )|(@@ ?$)', '', spm_model.decode(out_tokens))
